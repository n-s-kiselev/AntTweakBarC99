...
//              code. See Advanced_c99.c for the equivalent example ported
//              to strict C99.
//
//              SDL3 port of examples/glfw/Advanced_cpp.cpp - see
...
// by the display's content scale. We keep TwWindowSize/glViewport in pixel
// units (matching the real render target) and scale mouse coordinates from
// window/point space into that same pixel space before forwarding them to
// AntTweakBar. On a standard (non-HiDPI) display pixel size equals window
// size, so this scale is exactly 1.0 and everything behaves as before.
...
// same way main() scales the "Main" bar's. SDL only reports a single
// scalar (SDL_GetWindowDisplayScale), unlike GLFW's separate X/Y content
// scale, so both are set to the same value.
...
// ownership fight to route around the way examples/glfw/Advanced_cpp.cpp's
// own comment describes, so this is a plain cache, not a GLFW3-style shim.
...
static void DestroySDLCursorCache()
{
...
const char* title = "AntTweakBar example: Advanced (C++, SDL3)";
...
// TwKeyPressed(), same as examples/glfw/Advanced_cpp.cpp's charCallback does
...
{
    if (TwMouseButton(_Event->down ? TW_MOUSE_PRESSED : TW_MOUSE_RELEASED,
                       (TwMouseButtonID)_Event->button)) return;
...
{
    int winWidth = _PixelWidth, winHeight = _PixelHeight;
    SDL_GetWindowSize(_Window, &winWidth, &winHeight);
    g_MouseScaleX = (winWidth > 0) ? (double)_PixelWidth / winWidth : 1.0;
    g_MouseScaleY = (winHeight > 0) ? (double)_PixelHeight / winHeight : 1.0;
}

// Called on startup (with the window's actual initial pixel size) and again
// on every SDL_EVENT_WINDOW_PIXEL_SIZE_CHANGED - mirrors
// examples/glfw/Advanced_cpp.cpp's resizeCallback (registered as GLFW's
...
int main()
{
    ...
    // Fixed-function GL (glBegin/glEnd, display lists via glNewList/
    // glGenLists/glCallList throughout Scene) + AntTweakBar's TW_OPENGL
    // (compatibility, not Core Profile) renderer - request a plain 2.1
    // compatibility context (display lists are Core-Profile-incompatible),
    // the exact version validated by this backend's compile spike (see
    // docs/plans/sdl3-backend.md Step 1). The original GLFW3 example left
    // this unset (relying on GLFW's own compatibility-profile default);
    // requesting it explicitly here is the safer, equivalent choice for SDL3.

...
//              SDL3 port of examples/glfw/Triangle.c - see
...
// same reasoning as examples/glfw/Triangle.c's own g_MouseScaleX/Y, see
...
// ownership fight to route around the way examples/glfw/Triangle.c's own
...
// TwKeyPressed(), same as examples/glfw/Triangle.c's charCallback does per
...
// examples/glfw/Triangle.c's windowSizeCallback (registered as GLFW's
...
int main(void)
{
    ...
    SDL_Window *window = SDL_CreateWindow("AntTweakBar + SDL3 (Triangle)", g_Width, g_Height,
    ...
    // behavior) as examples/glfw/Triangle.c's own contentScaleX handling.

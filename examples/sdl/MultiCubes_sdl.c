...
//
//              SDL3 port of examples/glfw/MultiCubes.c - see
...
// ownership fight to route around the way examples/glfw/MultiCubes.c's own
...
// same reasoning as examples/glfw/MultiCubes.c's own g_MouseScaleX/Y, see
...
// Unlike most other ported examples, this one does NOT quit directly on
// Escape - it feeds TW_KEY_ESCAPE through to TwKeyPressed() like any other
// named key, because the "Quit" bar variable below is bound to key='ESC'
// (AntTweakBar itself toggles it when it sees that key), and main()'s loop
// condition is `!quit && running`. Preserves examples/glfw/MultiCubes.c's
// own keyCallback behavior exactly.
...
// TwKeyPressed(), same as examples/glfw/MultiCubes.c's charCallback does per
...
// examples/glfw/MultiCubes.c's windowSizeCallback (registered as GLFW's
...
int main(void)
{
    ...
    SDL_Window *window = SDL_CreateWindow("AntTweakBar + SDL3 (Multi Cubes)", g_Width, g_Height,
    ...
    // behavior) as examples/glfw/MultiCubes.c's own contentScaleX handling.

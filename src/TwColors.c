...
void ColorRGBToHLSi(int _R, int _G, int _B, int *_Hue, int *_Light, int *_Saturation)
{
    ...
    if(_Hue)        *_Hue       = (int)TClampFloat(h*(256.0f/360.0f), 0.0f, 255.0f);
    if(_Light)      *_Light     = (int)TClampFloat(l*256.0f, 0.0f, 255.0f);
    if(_Saturation) *_Saturation= (int)TClampFloat(s*256.0f, 0.0f, 255.0f);
}


// Local helper for ColorHLSToRGBf, below (was a local class with a static
// method in the original C++; C99 has no local classes/functions, so this
// is a plain file-scope static function instead).
static float HLSToRGBComponent(float _Rn1, float _Rn2, float _Huei)
{
    float hue = _Huei;
...
void ColorHLSToRGBf(float _Hue, float _Light, float _Saturation, float *_R, float *_G, float *_B)
{
    ...
    {
        if(_R) *_R = HLSToRGBComponent(rm1, rm2, rh+120);
        if(_G) *_G = HLSToRGBComponent(rm1, rm2, rh);
        if(_B) *_B = HLSToRGBComponent(rm1, rm2, rh-120);
    }
}
...
void ColorHLSToRGBi(int _Hue, int _Light, int _Saturation, int *_R, int *_G, int *_B)
{
    ...
    if(_R) *_R = (int)TClampFloat(r*256.0f, 0.0f, 255.0f);
    if(_G) *_G = (int)TClampFloat(g*256.0f, 0.0f, 255.0f);
    if(_B) *_B = (int)TClampFloat(b*256.0f, 0.0f, 255.0f);
}

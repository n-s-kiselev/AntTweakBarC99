...
// _Invert stores the logical negation of the parsed value, for an attribute
// defined as the inverse of the field it shares ("align_left" is stored as
// !m_AlignRight, see CTwVar in TwBar.h).
static int SetBoolAttrib(bool *_Field, const char *_Value, TwBar *_Bar, bool _Invert)
{
    bool NewValue;
...
//                                    _Var->m_Label = _Value;
    ...
    case V_ORDER:
        ...
        return 0;
    case V_VISIBLE:
        return SetBoolAttrib(&_Var->m_Visible, _Value, _Bar, false);
    case V_READONLY:
    ...
    case V_FULL_WIDTH:
        return SetBoolAttrib(&_Var->m_FullWidth, _Value, _Bar, false);
    case V_ALIGN_RIGHT:
        return SetBoolAttrib(&_Var->m_AlignRight, _Value, _Bar, false);
    case V_ALIGN_LEFT:
        return SetBoolAttrib(&_Var->m_AlignRight, _Value, _Bar, true);
    default:
...
// Left edge of the widget drawn on row _Level of _Var: the value column as always, or - for
// a "full_width" var - the label column too, indented by the row's own nesting level with
// the same formula CTwBar_Draw's custom-type rendering and row-highlight rect use.
...
static void CTwBar_MultilineUpdateScrollThumb(const CTwBar *_Bar, struct CTwMultilineVal *_ML, int _BlockY0, int _BlockY1)
{
    ...
    int htrack = CTwBar_MultilineTrackHeight(_Bar, _BlockY0, _BlockY1);

    int hscrml = (_ML->m_NbTextLines>0) ? ((htrack*_ML->m_NbLines)/_ML->m_NbTextLines) : htrack;
    if( hscrml<=4 )
        hscrml = 4;
    if( hscrml>htrack )
        hscrml = htrack;
    int yscrml = (_ML->m_NbTextLines>0) ? ((htrack*_ML->m_FirstTextLine)/_ML->m_NbTextLines) : 0;
    if( yscrml<0 )
        yscrml = 0;
    if( yscrml>htrack-4 )
        yscrml = htrack-4;
    ...
    _ML->m_ScrollY0 = _BlockY0+sw+yscrml;
    _ML->m_ScrollY1 = _BlockY0+sw+yscrml+hscrml;
}
...
static const CSdsArray *CTwBar_MultilineWrapText(CTwMultilineWrapCache *_Cache, CTwVarAtom *_Atom, const char *_Text, int _WrapWidth, const CTexFont *_Font)
{
    ...
    if( !CacheHit )
    {
        ...
        SplitString(&_Cache->m_Lines, _Text, _WrapWidth, _Font, &_Cache->m_LineStarts, &_Cache->m_LineEnds);
...
static void CTwBar_MultilineSourceRangeAt(const CIntArray *_Starts, const CIntArray *_Ends, int _SourceLen, int _LineIdx, int *_Start, int *_End)
{
    ...
    {
        *_Start = _Starts->items[_LineIdx];
        *_End = _Ends->items[_LineIdx];
    }
    else
    {
        *_Start = _SourceLen;
...
{
    int Row = 0;
    for( size_t k=1; k<_Starts->count; ++k )
    {
        if( _Starts->items[k]>_Offset )
            break;
        Row = (int)k;
    }
    return Row;
...
void CTwBar_ListLabels(CTwBar *_Bar, CSdsArray *_Labels, CColor32Array *_Colors, CColor32Array *_BgColors, bool *_HasBgColors, const CTexFont *_Font, int _AtomWidthMax, int _GroupWidthMax)
{
    ...
    for( int h=0; h<nh; ++h )
    {
        bool AlignRight = false; // only the plain-label case below (align_right/align_left) sets this
        ...
        // A "full_width" atom is exempt: its own wrapped text, not a one-time label, is
        // what the label column carries, so every row of its block has content to show.
        ...
        {
            // "full_width": no separate label - an atom shows its own value text through the
            // label column instead (empty for buttons and custom types, which correctly leaves
            // their row blank). A group has no textual value of its own, so its row stays
            // blank too; whatever it does draw (a color swatch) is widened in CTwBar_Draw.
            CTwVar *FWVar = _Bar->m_HierTags.items[h].m_Var;
            Text = NULL;
            Len = 0;
            if( !CTwVar_IsGroup(FWVar) )
            {
                ...
                if( IsMultilineTextVar(FWVar) )
                {
                    ...
                    if( WrappedLine!=NULL )
                    {
                        Text = (const unsigned char *)WrappedLine;
                        Len = (int)sdslen(WrappedLine);
                    }
                }
                else
                {
        ...
        if( !IsCustom )
        {
            ...
            if( AlignRight )
            {
                // "align_right": every row is drawn through one fixed-position DrawText call,
    ...
    for( int h=0; h<nh; ++h )
            ...
            || (CTwVar_IsGroup(_Bar->m_HierTags.items[h].m_Var) && ((const CTwVarGroup *)_Bar->m_HierTags.items[h].m_Var)->m_SummaryCallback!=NULL) )
        {
            ...
            if( !CTwVar_IsGroup(_Bar->m_HierTags.items[h].m_Var) )
            {
                ...
                // "full_width" atom: CTwBar_ListLabels wraps it instead, at the wider
                // label-column width, and wrapping it twice would fight over the same cached
            ...
            if( !HasBgColor )
                ...
                tw_da_append(_BgColors, (color32)0x00000000); // no background at the narrow value-column position the now-blank value text no longer occupies
...
int CTwBar_ComputeValuesWidth(CTwBar *_Bar, const CTexFont *_Font)
{
    ...
    for( int h=0; h<nh; ++h )
        ...
        {
            Atom = (const CTwVarAtom *)_Bar->m_HierTags.items[h].m_Var;
            // A "full_width" atom's value renders through the label column, never here, so it
            // must not inflate this auto-fit width. A bar holding only full_width atoms lands
            // at ValuesWidth==0 by design: all horizontal space goes to the label column,
            // maximizing their own wrap width.
...
void CTwBar_Update(CTwBar *_Bar)
{
    ...
    // interactive is decided separately, by CTwBar_MultilineNeedsScrollbar.
    {
        ...
        for( int hml=0; hml<nhml; ++hml )
        {
            ...
            CTwBar_MultilineUpdateScrollThumb(_Bar, &MLAtom->m_Val.m_Multiline, by0, by1);
        }
    }
...
void CTwBar_Draw(CTwBar *_Bar, int _DrawPart)
{
    ...
    if( !_Bar->m_IsMinimized )
    {
        ...
        if( _DrawPart&DRAW_CONTENT )
        {
            ...
            for( h=0; h<nh; ++h )
            {
                // Every widget X below reads vx0 rather than m_VarX1 directly, so "full_width"
                // widens each widget kind uniformly - see CTwBar_RowWidgetX0.
                ...
                else if( ((CTwVarAtom *)_Bar->m_HierTags.items[h].m_Var)->m_Type==TW_TYPE_BUTTON && !_Bar->m_IsPopupList )
                {
                    ...
                    {
                        // Halve the width, not the sum of the two absolute edges: only the
                        // former truncates the same way the old m_PosX+(m_VarX1+m_VarX2)/2 did
                        // once m_PosX goes negative (bar dragged off the left of the window).
                        int cbmid = vx0+(_Bar->m_PosX+_Bar->m_VarX2-vx0)/2;
                        cbx0 = cbmid-bw/2+1;
                        cbx1 = cbmid+bw/2-1;
                    }
                    else
                ...
                else if( IsEnumType(Var->m_Type) && !Var->m_ReadOnly && !g_TwMgr->m_IsRepeatingMousePressed && OnFocus )
                {
                         ...
                         || (!Var->m_ReadOnly && CTwBar_EditInPlaceAcceptVar(_Bar, Var)) )
                    {
                        ...
                        if( !_Bar->m_EditInPlace.m_Active || _Bar->m_EditInPlace.m_Var!=Var )
                        {
                            ...
                            // CTwBar_RowWidgetX0 returns an absolute screen X, but
                            // CTwBar_EditInPlaceStart's _X is bar-relative like m_VarX1, so m_PosX
...
static void CTwBar_EditInPlaceDrawMultiline(CTwBar *_Bar)
{
    ...
    else if( CaretRow>=ML->m_FirstTextLine+ML->m_NbLines )
    ...
    int SelMin = min(_Bar->m_EditInPlace.m_CaretPos, _Bar->m_EditInPlace.m_SelectionStart);
    int SelMax = max(_Bar->m_EditInPlace.m_CaretPos, _Bar->m_EditInPlace.m_SelectionStart);
    ...
    for( int r=0; r<ML->m_NbLines; ++r )
    {
        ...
        g_TwMgr->m_Graph->DrawText(g_TwMgr->m_Graph, _Bar->m_EditInPlace.m_EditTextObj, RowX, RowY, ColText, ColBg);
        sdsfree(RowStr);
...
void CTwBar_EditInPlaceDraw(CTwBar *_Bar)
{
    ...
    {
        CTwBar_EditInPlaceDrawMultiline(_Bar);
        return;
    }
...
bool CTwBar_EditInPlaceMouseMove(CTwBar *_Bar, int _X, int _Y, bool _Select)
{
    ...
    if ( _Y<_Bar->m_PosY+_Bar->m_EditInPlace.m_Y || _Y>_Bar->m_PosY+_Bar->m_EditInPlace.m_Y+_Bar->m_Font->m_CharHeight )
        return false;
